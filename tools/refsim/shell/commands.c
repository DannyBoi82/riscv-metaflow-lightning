...
static void run_simulator(cpu_state_t *cpu_state)
{
    if (cpu_state->verbose_mode && cpu_state->trace_mode) {
      ...
      fprintf(cpu_state->trace_fd, "  \"pc\": 0x%08x,\n", cpu_state->pc);
      for (int i = 0; i < (int)array_len(cpu_state->registers); i++)
      {
	fprintf(cpu_state->trace_fd, "  \"x%d\": 0x%08x,\n", i, register_read(cpu_state,i));
      }

      fprintf(cpu_state->trace_fd, "  \"inst\": 0x%08x,\n", mem_read32(cpu_state, cpu_state->pc));
    }
    ...
    if (cpu_state->verbose_mode) {
      if (cpu_state->trace_mode) {
	fprintf(cpu_state->trace_fd, "  \"nocomma\" : \"nocomma\"\n");
	fprintf(cpu_state->trace_fd, "}\n");
      } else {
...
/**
 * Toggles register file write tracing  mode for the simulator.
 *
 * If tracing mode is active, then the simulator writes a trace of register
 * file updates to the file "trace.txt".  Toggling tracing mode restarts the file.
 * This rf write trace is used by the new debugging mode that checks the correctness
 * of  each rf write during RTL sim.
 **/
void command_trace(cpu_state_t *cpu_state, char *args[], int num_args)
{
    ...
    // Check that the appropriate number of arguments was specified
    if (num_args != TRACE_NUM_ARGS) {
        fprintf(stderr, "Error: Improper number of arguments specified to "
                "'trace' command.\n");
        return;
    }

    // Toggle the trace  mode for the processor
    cpu_state->trace_mode = !cpu_state->trace_mode;
    if (cpu_state->trace_mode) {
      cpu_state->trace_fd = fopen("trace.txt", "w");
      if (!cpu_state->trace_fd) {
	perror("Failed to open rf write trace file");
	exit(1);
      }
    } else {
...
void command_help(cpu_state_t *cpu_state, char *args[], int num_args)
{
    ...
    print_help("v[erbose]", "Toggles verbose mode for the simulator. When "
            "active, the simulator dumps the registers to stdout after each cycle.");
    print_help("t[race]", "Toggles trace mode for the simulator. When "
